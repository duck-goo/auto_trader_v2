"""Repository for persisted 15-minute intraday bars."""
...
class IntradayBar15mRepository:
    """Pure DB repository for stored 15-minute bars."""
    ...
    def replace_for_symbol_and_date(
        ...
        *,
        trade_date: str,
        symbol: str,
        bars: Sequence[IntradayBar15m],
        refreshed_at: str,
    ) -> list[IntradayBar15mRow]:
        ...
        normalized_bars: list[IntradayBar15m] = []
        seen_bar_start_at: set[str] = set()

        for bar in bars:
            if not isinstance(bar, IntradayBar15m):
                raise ValueError(
                    "bars must contain only IntradayBar15m instances."
                )
            ...
            if end_dt <= start_dt:
                raise ValueError(
                    "bar_end_at must be later than bar_start_at: "
                    f"{bar_start_at!r} >= {bar_end_at!r}"
                )

            if start_dt.strftime("%Y-%m-%d") != normalized_trade_date:
                raise ValueError(
                    "bar_start_at trade_date mismatch: "
                    f"expected={normalized_trade_date}, actual="
                    f"{start_dt.strftime('%Y-%m-%d')}"
                )

            if bar_start_at in seen_bar_start_at:
                raise ValueError(
                    f"Duplicate 15-minute bar_start_at: {bar_start_at!r}"
                )
    ...
    def list_for_symbol_and_date(
        ...
        *,
        ...
        rows = self._conn.execute(
            ...
            FROM intraday_bars_15m
            WHERE trade_date = ? AND symbol = ?
            ORDER BY bar_start_at ASC
            """,
            (normalized_trade_date, normalized_symbol),
        ).fetchall()
        return RowMapper.map_many(rows, IntradayBar15mRow)
    ...
    def list_recent_for_symbol(
        ...
        *,
        ...
        mapped = RowMapper.map_many(rows, IntradayBar15mRow)
        mapped.reverse()
        return mapped
