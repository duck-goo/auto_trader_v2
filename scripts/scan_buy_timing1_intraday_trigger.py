"""
Poll current prices for buy timing1 intraday breakout triggers.
...
from strategy import Timing1IntradayTriggerSettings
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan buy timing1 intraday trigger from stored convergence signals."
    )
    parser.add_argument(
        "--trade-date",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="How many daily candles to request for next-trading-day validation. Default: 5",
    )
    parser.add_argument(
        "--write",
        action="store_true",
        help="Actually record intraday trigger signals into SQLite.",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def main() -> int:
    ...
    strategy_settings = Timing1IntradayTriggerSettings(
        start_time=args.start_time,
        cutoff_time=args.cutoff_time,
    )

    _section("Scan Buy Timing1 Intraday Trigger")
