...
class KisClient:
    ...
    def request_get(
        ...
        *,
        ...
        - 네트워크 예외 시 자동 재시도 (settings.request_retry_count)
        - HTTP 401 시 토큰 강제 재발급 후 정확히 1회 재시도 (안전망)
        - rt_cd != "0" 시 KisApiError (재시도 안 함)
        """
        ...
        try:
            ...
            # 401 안전망: 토큰 강제 재발급 후 1회만 재시도
            self._log.warning(
                f"401 감지, 토큰 강제 재발급 후 1회 재시도: {e}"
            )
            try:
    ...
    def _execute_get(
        ...
        rt_cd 에러는 재시도 안 함 (KIS가 명시적으로 거부한 것).
        HTTP 401도 여기서 잡지 않고 위로 전파 (request_get이 안전망 처리).
        """
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                self._log.debug(
                    f"GET {url} tr_id={tr_id} "
                    f"params={params} headers={self._mask_secret(headers)}"
                )
                ...
                self._error_log.warning(
                    f"GET 네트워크 실패 (시도 {attempt}/{max_attempts}) "
                    f"tr_id={tr_id}: {e}"
                )
        ...
        raise KisApiError(
            f"GET 네트워크 실패 (최종 {max_attempts}회): {last_error}",
            tr_id=tr_id,
        )
