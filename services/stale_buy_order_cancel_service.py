"""Cancel stale unresolved buy orders after a timeout."""
...
from services.order_service import CancelOutcome, OrderService
...
class StaleBuyOrderCancelOutcome(str, enum.Enum):
...
class StaleBuyOrderCancelSettings:
    timeout_seconds: int

    def validated(self) -> "StaleBuyOrderCancelSettings":
        return StaleBuyOrderCancelSettings(
...
@dataclass(frozen=True)
class StaleBuyOrderCancelCandidate:
    client_order_id: str
    symbol: str
    status: str
    requested_at: str
    age_seconds: int
    outcome: StaleBuyOrderCancelOutcome
...
@dataclass(frozen=True)
class StaleBuyOrderCancelResult:
    trade_date: str
    scanned_at: str
    ...
    blocked_count: int
    acted_count: int
    candidates: tuple[StaleBuyOrderCancelCandidate, ...]


class StaleBuyOrderCancelService:
    """Find stale unresolved buy orders and optionally cancel them."""
    ...
    def cancel_stale_orders(
        ...
        *,
        ...
        candidates: list[StaleBuyOrderCancelCandidate] = []
        skip_ids = (
            frozenset()
        ...
        _log.info(
            f"[stale_buy_cancel:start] trade_date={trade_date} "
            f"unresolved_order_count={len(unresolved_orders)} "
            f"execute_cancels={execute_cancels} "
            f"skip_count={len(skip_ids)}"
        )
        ...
        preview_ready_count = sum(
            ...
            if item.outcome == StaleBuyOrderCancelOutcome.PREVIEW_READY
        )
        skipped_count = sum(
            1 for item in candidates if item.outcome == StaleBuyOrderCancelOutcome.SKIPPED
        )
        cancelled_count = sum(
            1 for item in candidates if item.outcome == StaleBuyOrderCancelOutcome.CANCELLED
        )
        rejected_count = sum(
            1 for item in candidates if item.outcome == StaleBuyOrderCancelOutcome.REJECTED
        )
        unknown_count = sum(
            1 for item in candidates if item.outcome == StaleBuyOrderCancelOutcome.UNKNOWN
        )
        blocked_count = sum(
            1 for item in candidates if item.outcome == StaleBuyOrderCancelOutcome.BLOCKED
        )
        acted_count = sum(1 for item in candidates if item.acted)

        _log.info(
            f"[stale_buy_cancel:done] trade_date={trade_date} "
        ...
        return StaleBuyOrderCancelResult(
            trade_date=trade_date,
        ...
        *,
        ...
        if row.side != "buy":
            return self._build_skipped(
                row=row,
                age_seconds=age_seconds,
                reason_code="NON_BUY_ORDER",
                reason_message="Only buy orders are handled by this cancel service.",
            )
        ...
        if not execute_cancels:
            return StaleBuyOrderCancelCandidate(
                client_order_id=row.client_order_id,
                symbol=row.symbol,
                status=row.status.value,
                requested_at=row.requested_at,
                age_seconds=age_seconds,
                outcome=StaleBuyOrderCancelOutcome.PREVIEW_READY,
                reason_code=None,
                reason_message=None,
                acted=False,
            )
        ...
        try:
            ...
            raise ServiceError(
                f"Failed to cancel stale buy order: "
        ...
        return StaleBuyOrderCancelCandidate(
            ...
            requested_at=row.requested_at,
            age_seconds=age_seconds,
            outcome=StaleBuyOrderCancelOutcome(cancel_result.outcome.value),
    ...
    def _build_skipped(
        *,
        ...
        return StaleBuyOrderCancelCandidate(
            ...
            requested_at=row.requested_at,
            age_seconds=age_seconds,
            outcome=StaleBuyOrderCancelOutcome.SKIPPED,
