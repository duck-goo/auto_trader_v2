"""Read-only scan service for buy timing 1 daily setup."""
...
from strategy import Timing1SetupEvaluator, Timing1SetupMatch, Timing1SetupSettings


STRATEGY_NAME_TIMING1_SETUP = "buy_timing1_setup"
...
class Timing1SetupScanCandidate:
    symbol: str
    name: str
    market: str
...
class Timing1SetupScanResult:
    ...
    candidates: tuple[Timing1SetupScanCandidate, ...]
...
class Timing1SetupScanService:
    """
    Scan the stored universe and record read-only timing1 setup signals.
    ...
    def __init__(
        ...
        *,
        ...
        evaluator: Timing1SetupEvaluator | None = None,
    ) -> None:
        self._broker = broker
        self._conn = conn
        ...
        self._evaluator = evaluator or Timing1SetupEvaluator()
    ...
    def scan(
        ...
        *,
        ...
        candidates: list[Timing1SetupScanCandidate] = []
        payloads_to_record: list[dict] = []

        _log.info(
            f"[timing1_setup_scan:start] trade_date={trade_date} "
        ...
        for row in rows:
            ...
            try:
                ...
                raise ServiceError(
                    f"Failed to evaluate timing1 setup for symbol={row.symbol}: "
                    f"{type(exc).__name__}: {exc}"
                ) from exc
    ...
    def _validate_daily_count(
        ...
        *,
        ...
        if value < minimum:
            raise ValueError(
                f"daily_count must be >= {minimum} for timing1 setup: {value!r}"
            )
        return value
    ...
    def _has_existing_signal(self, *, symbol: str, trade_date: str) -> bool:
        ...
        for row in existing:
            if row.strategy_name != STRATEGY_NAME_TIMING1_SETUP:
                continue
            if not row.payload:
                continue
    ...
    def _build_payload(
        ...
        *,
        ...
        return {
            ...
            "ma20_now": round(match.ma_short_now, 6),
            "ma20_past": round(match.ma_short_past, 6),
            "ma60_now": round(match.ma_long_now, 6),
            "ma60_past": round(match.ma_long_past, 6),
            "strong_day": {
                "date": match.strong_day.date,
                "open_price": match.strong_day.open_price,
                "close_price": match.strong_day.close_price,
                "prev_close": match.strong_day.prev_close,
                "gain_rate": round(match.strong_day.gain_rate, 6),
                "volume": match.strong_day.volume,
                "avg_volume_before": match.strong_day.avg_volume_before,
                "volume_ratio": round(match.strong_day.volume_ratio, 6),
            },
        }
